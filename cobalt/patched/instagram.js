...
export default function instagram(obj) {
    ...
    function extractOldPost(data, id, alwaysProxy) {
        ...
        const caption = shortcodeMedia?.edge_media_to_caption?.edges?.[0]?.node?.text || undefined;
        ...
        if (sidecar) {
            ...
            if (picker.length) return { picker, caption }
        }
    ...
    function extractNewPost(data, id, alwaysProxy) {
        ...
        const caption = data?.caption?.text || undefined;
        if (carousel) {
    ...
    async function getStory(username, id) {
        ...
        const caption = item?.caption?.text || undefined;
