...
def _clip_caption(text: str | None) -> str | None:
    """Подпись поста от пропатченного cobalt (поле `caption`)."""
    if not text:
        return None
    text = text.strip()
    if not text:
...
def _cobalt_resolve(
    ...
    """Спрашивает cobalt. Возвращает (items, caption, error), где items —
    список {"type": "video"|"photo"|"auto", "url": str}. Несколько
    элементов = карусель. caption — текст поста (наш патч cobalt)."""
    ...
    try:
        ...
        try:
            ...
            return None, None, "Не удалось обработать ссылку."
    except Exception as exc:
        logger.warning("cobalt unreachable for %s: %s", url, str(exc)[:200])
        return None, None, "Сервис скачивания недоступен (попробуй позже)."

    status = data.get("status")
    caption = _clip_caption(data.get("caption"))
    if status in ("tunnel", "redirect"):
        u = data.get("url")
        if not u:
            return None, None, "Нечего скачивать (пустой ответ)."
        # Тип определим по Content-Type при скачивании.
        return [{"type": "auto", "url": u}], caption, None
    if status == "picker":
        ...
        if not items:
            return None, None, "Нечего скачивать (пустой ответ)."
        return items[:_MAX_GROUP], caption, None
    if status == "local-processing":
        return None, None, (
            "Этот ролик требует склейки на клиенте — не поддерживается."
        )
    ...
    if "auth" in err or "private" in err or "login" in err:
        return None, None, "Видео приватное / требует логина."
    if "unsupported" in err or "link" in err:
        return None, None, "Эта ссылка не поддерживается для скачивания."
    return None, None, "Не удалось получить видео (сайт не отдал)."
...
def download_sync(
    ...
    Возвращает (items, caption, error), items — список (path, type),
    caption — текст поста или None.
    Вызывать в asyncio.to_thread — операция блокирующая и долгая."""
    resolved, caption, err = _cobalt_resolve(url)
    if err or not resolved:
        return None, None, err or "Не удалось получить видео."
    ...
    if not out:
        return None, None, last_err or "Не удалось скачать файл."
    return out, caption, None
